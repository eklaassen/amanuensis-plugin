...
RSpec.describe Amanuensis::UploadsApiController, type: :request do
  ...
  def stub_presign(
    ...
    stub_request(:post, "https://amanuensis.example.com/v1/plugin/uploads").with(
      headers: {
        "Authorization" => "Bearer #{secret_for(credential)}",
      },
    ).to_return(
      status: status,
      headers: {
  ...
  def stub_complete(id: "upl_1", status: 201, body: { meeting_id: "upl_1" })
    ...
    stub_request(:post, "https://amanuensis.example.com/v1/plugin/uploads/#{id}/complete").with(
      headers: {
        "Authorization" => "Bearer #{secret_for(credential)}",
      },
    ).to_return(
  ...
  describe "#upload_config" do
    ...
    it "returns the size cap and allowed extensions for a writer" do
      ...
      expect(response.parsed_body["allowed_extensions"]).to eq(
        Amanuensis::UploadPolicy::ALLOWED_EXTENSIONS,
      )
    end
  end
  ...
  describe "#create" do
    ...
    it "authenticates upstream with the admin key, not the read secret" do
      ...
      credential =
        Amanuensis::PluginContractFixture.credential_for(method: "POST", path: "/v1/plugin/uploads")
      expect(
        a_request(:post, "https://amanuensis.example.com/v1/plugin/uploads").with(
          headers: {
            "Authorization" => "Bearer #{secret_for(credential)}",
          },
        ),
    ...
    it "sends a request body that matches the contract schema for this route" do
      ...
      violations =
        Amanuensis::PluginContractFixture.schema_violations(
          method: "POST",
  ...
  describe "#complete" do
    ...
    it "authenticates the complete call upstream with the admin key, not the read secret" do
      ...
      expect(
        a_request(:post, "https://amanuensis.example.com/v1/plugin/uploads/upl_1/complete").with(
          headers: {
            "Authorization" => "Bearer #{secret_for(credential)}",
          },
        ),
    ...
    it "refuses an upload id with characters outside the allowed format" do
      ...
      stub_request(:post, /amanuensis\.example\.com/).to_return(status: 201, body: "{}")
