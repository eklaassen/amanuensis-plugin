...
RSpec.describe Amanuensis::OutcomesApiController, type: :request do
  ...
  def stub_outcomes(status:, meetings: [], has_more: false)
    stub_request(
      :get,
      %r{\Ahttps://amanuensis\.example\.com/v1/plugin/meetings\?.*status=#{status}.*\z},
    ).to_return(
      status: 200,
      headers: {
  ...
  describe "#index" do
    ...
    it "filters to failed meetings and reports the failure reason" do
        ...
        meetings: [
          {
            ...
            "recorded_at" => "2026-07-02T19:00:00Z",
            "status" => "failed",
            "failure_reason" => "Upstream transcription service unavailable",
          },
        ],
      )
      ...
      expect(body["meetings"].first["failure_reason"]).to eq(
        "Upstream transcription service unavailable",
      )
    end
    ...
    it "does not filter by canon_status on the failed tab -- failed meetings never get a proposal" do
      ...
      expect(response.status).to eq(200)
      expect(
        a_request(:get, %r{\Ahttps://amanuensis\.example\.com/v1/plugin/meetings}).with do |req|
    ...
    it "surfaces an error when the upstream API fails" do
      stub_request(:get, %r{\Ahttps://amanuensis\.example\.com/v1/plugin/meetings\?.*\z}).to_return(
        status: 500,
        body: "boom",
      )
