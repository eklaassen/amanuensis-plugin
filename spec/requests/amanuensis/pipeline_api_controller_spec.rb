...
RSpec.describe Amanuensis::PipelineApiController, type: :request do
  ...
  def stub_active(meetings: [])
    stub_request(:get, "https://amanuensis.example.com/v1/plugin/pipeline/active").to_return(
      status: 200,
      headers: {
  ...
  describe "#active" do
    ...
    it "notes the attempt number when a meeting has retried" do
      ...
      expect(response.parsed_body["stage_groups"].first["meetings"].first["attempt_note"]).to eq(
        "attempt 3",
      )
    end
