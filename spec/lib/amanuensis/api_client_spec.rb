...
RSpec.describe Amanuensis::ApiClient do
  ...
  describe '.reader' do
    it 'authenticates with the reader secret and never the admin key' do
      stub_request(:get, 'https://amanuensis.example.com/v1/plugin/meetings')
        .with(headers: { 'Authorization' => 'Bearer reader-secret' })
      ...
      expect(
        a_request(:get, 'https://amanuensis.example.com/v1/plugin/meetings')
          .with(headers: { 'Authorization' => 'Bearer reader-secret' })
      ).to have_been_made
      expect(
        a_request(:get, 'https://amanuensis.example.com/v1/plugin/meetings')
  ...
  describe 'verb coverage' do
    it 'sends GET requests with encoded query params' do
      stub_request(:get, 'https://amanuensis.example.com/v1/plugin/meetings?limit=25')
        .to_return(status: 200, body: '{"meetings":[]}', headers: { 'Content-Type' => 'application/json' })
    ...
    it 'sends POST requests with a JSON body' do
      stub =
        stub_request(:post, 'https://amanuensis.example.com/v1/plugin/meetings')
          .with(body: { title: 'New meeting' }.to_json, headers: { 'Content-Type' => 'application/json' })
    ...
    it 'sends PATCH requests with a JSON body' do
      stub =
        stub_request(:patch, 'https://amanuensis.example.com/v1/plugin/meetings/abc123')
          .with(body: { title: 'Renamed' }.to_json)
          .to_return(status: 200, body: '{"id":"abc123"}', headers: { 'Content-Type' => 'application/json' })
    ...
    it 'sends DELETE requests with encoded query params' do
      stub =
        stub_request(:delete, 'https://amanuensis.example.com/v1/plugin/meetings/abc123?force=true')
  ...
  describe 'rescued transport errors' do
    [
      ...
      it "rescues #{error_class} into an error Result instead of raising" do
        stub_request(:get, 'https://amanuensis.example.com/v1/plugin/meetings').to_raise(error_class)
    ...
    it 'does not rescue errors outside the deliberate list' do
      stub_request(:get, 'https://amanuensis.example.com/v1/plugin/meetings').to_raise(ArgumentError)
  ...
  describe 'missing configuration handling' do
    it 'returns an error Result without making a request when the API URL is blank' do
      ...
      expect(result.ok?).to eq(false)
      expect(result.error).to be_present
      expect(a_request(:get, %r{amanuensis\.example\.com})).not_to have_been_made
    end
