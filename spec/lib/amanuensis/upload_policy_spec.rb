...
RSpec.describe Amanuensis::UploadPolicy do
  ...
  describe '.sanitize_filename' do
    ...
    it 'caps length' do
      expect(described_class.sanitize_filename("#{'a' * 400}.mp3").length)
        .to be <= described_class::MAX_FILENAME_LENGTH
    end
  end

  describe '.validate!' do
    it 'passes a valid audio file' do
      expect { described_class.validate!(filename: 'talk.mp3', size_bytes: 1024) }.not_to raise_error
    end

    it 'rejects a blank filename' do
      expect { described_class.validate!(filename: '', size_bytes: 1024) }
        .to raise_error(described_class::Rejected)
    end

    it 'rejects a disallowed extension' do
      expect { described_class.validate!(filename: 'notes.pdf', size_bytes: 1024) }
        .to raise_error(described_class::Rejected, /not allowed/)
    end

    it 'rejects a zero-byte file' do
      expect { described_class.validate!(filename: 'talk.mp3', size_bytes: 0) }
        .to raise_error(described_class::Rejected)
    end

    it 'rejects a file over the cap' do
      expect { described_class.validate!(filename: 'talk.mp3', size_bytes: described_class::MAX_BYTES + 1) }
        .to raise_error(described_class::Rejected, /too large/)
    end

    it 'accepts a file exactly at the cap' do
      expect { described_class.validate!(filename: 'talk.mp3', size_bytes: described_class::MAX_BYTES) }
        .not_to raise_error
    end
  end
end
